...
summary: A mídia de inicialização do seu sistema de armazenamento FAS50 armazena dados essenciais de firmware e configuração. O processo de substituição envolve a remoção do módulo controlador, a remoção da mídia de inicialização danificada, a instalação da mídia de inicialização de substituição e a reinstalação do módulo controlador. 
---
= Substituir a mídia de inicialização para recuperação de inicialização automatizada - FAS50
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
