...
summary: Para concluir a substituição do controlador do seu sistema FAS50, primeiro restaure a configuração de NetApp Storage Encryption, se necessário, e instale as licenças necessárias no novo controlador. Em seguida, confirme se as interfaces lógicas estão relatando para suas portas iniciais e execute uma verificação de integridade. Por fim, registre o número de série do novo controlador e depois devolva a peça com defeito para a NetApp. 
---
= Substituição completa do controlador - FAS50
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Para concluir a substituição do controlador para o seu sistema FAS50, primeiro restaure a configuração de encriptação de armazenamento NetApp (se necessário) e instale as licenças necessárias no novo controlador. Em seguida, confirme se as interfaces lógicas (LIFs) estão relatando para suas portas domésticas e execute uma verificação de integridade do cluster. Por fim, registe o número de série do novo controlador e, em seguida, devolva a peça com falha ao NetApp.
