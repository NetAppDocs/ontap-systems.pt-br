...
summary: 'Após concluir a substituição da FRU em uma configuração MetroCluster de dois nós, você pode executar a operação de reversão do MetroCluster . Isso restaura a configuração ao seu estado operacional normal, com as máquinas virtuais de armazenamento de origem de sincronização (SVMs) no site anteriormente comprometido agora ativas e fornecendo dados dos pools de discos locais. O sistema AFF A700 suporta apenas procedimentos manuais de recuperação de mídia de inicialização.' 
---
= Alternar entre agregados em uma configuração MetroCluster de dois nós - AFF A700
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Após concluir a substituição da mídia de inicialização, execute a operação de reversão do MetroCluster . O sistema AFF A700 suporta apenas procedimentos manuais de recuperação de mídia de inicialização. A recuperação automática de mídia de inicialização não é suportada.
