...
summary: 'A mídia de inicialização armazena um conjunto primário e um conjunto secundário de arquivos de sistema (imagem de inicialização) que o sistema usa ao inicializar. O sistema AFF A300 suporta apenas procedimentos manuais de recuperação de mídia de inicialização. A recuperação automática de mídia de inicialização não é suportada. Dependendo da configuração da sua rede, você pode realizar uma substituição sem interrupção ou com interrupção.' 
---
