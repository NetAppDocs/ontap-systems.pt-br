...
summary: Antes de desligar o controlador com defeito e verificar o status das chaves de criptografia integradas, você deve verificar o status do controlador com defeito, desativar a devolução automática e verificar qual versão do ONTAP o sistema está executando. O sistema AFF A300 suporta apenas procedimentos manuais de recuperação de mídia de inicialização. 
---
= Verifique o suporte à criptografia na sua mídia de inicialização no seu sistema AFF A300
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Para garantir a segurança dos dados no seu sistema de armazenamento, você precisa verificar o suporte e o status da chave de criptografia na sua mídia de inicialização. O sistema AFF A300 suporta apenas procedimentos manuais de recuperação de mídia de inicialização. A recuperação automática de mídia de inicialização não é suportada.

Verifique se a sua versão do ONTAP é compatível com a Criptografia de Volume NetApp (NVE) e, antes de desligar o controlador, verifique se o gerenciador de chaves está ativo.
