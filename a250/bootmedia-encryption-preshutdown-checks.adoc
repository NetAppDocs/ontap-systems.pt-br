...
---
= Verifique o suporte à criptografia na sua mídia de inicialização no seu sistema AFF A250
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Para garantir a segurança dos dados no seu sistema de storage AFF A250, você precisa verificar o suporte à chave de criptografia e o status na sua mídia de inicialização. Verifique se a sua versão do ONTAP suporta NetApp Volume Encryption (NVE) e, antes de desligar o controlador, verifique se o gerenciador de chaves está ativo.

Se o seu sistema de storage estiver executando ONTAP 9.18.1 ou posterior, use o link:bootmedia-replace-workflow-bmr.html["procedimento automatizado de recuperação de inicialização"]. Se o seu sistema estiver executando uma versão anterior do ONTAP, você deve usar o processo de recuperação de inicialização manual.
