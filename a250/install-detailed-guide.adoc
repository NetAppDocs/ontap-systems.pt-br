...
---
= Instale seu sistema AFF A250 usando instruções detalhadas passo a passo
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Instale seu sistema de storage AFF A250 seguindo estas instruções detalhadas passo a passo, desde a preparação do local até a inicialização do cluster ONTAP.
...
.Passos
. Use o cabo de interconexão de cluster de 25 GbE para conectar as portas de interconexão de cluster e0c a e0c e e0d a e0d.
+
...
NOTE: link:https://hwu.netapp.com["NetApp Hardware Universe"^]A prioridade de slot para placas de rede do host (Fibre Channel ou 25 GbE) é o slot 2. No entanto, se você tiver ambas as placas, a placa Fibre Channel vai no slot 2 e a placa 25 GbE vai no slot 1 (como mostrado nas opções abaixo). Se você tiver um shelf externo, a placa de storage vai no slot 1, o único slot compatível com shelves.
...
--
.Opção 2: cabo para rede de dados ou de host de 25 GbE
--
As portas 25 GbE dos controladores são conectadas a switches de dados ou de rede de host de 25 GbE.
...
NOTE: Para obter informações sobre como estabelecer conectividade contínua de uma porta de console do controlador para gerenciamento remoto, consulte o artigo da Base de Conhecimento. https://kb.netapp.com/on-prem/ontap/OHW/OHW-KBs/How_to_connect_an_ONTAP_controller_console_port_for_remote_management["Como conectar uma porta de console do controlador ONTAP para gerenciamento remoto"^].
