...
summary: Antes de substituir a mídia de inicialização em seu AFF A20, AFF A30 e AFF A50 sistema de storage, certifique-se de atender aos requisitos necessários para uma substituição bem-sucedida. Isso inclui verificar se você possui a mídia de inicialização de substituição correta, confirmar se a porta e0M no controlador com defeito está funcionando corretamente e determinar se o Onboard Key Manager ou o External Key Manager está ativado. 
---
= Analise os requisitos para recuperação automática da mídia de inicialização no seu sistema AFF A20, AFF A30 ou AFF A50
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
