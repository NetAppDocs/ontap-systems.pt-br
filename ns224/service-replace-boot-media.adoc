...
summary: Você pode substituir uma Mídia de inicialização com falha em uma gaveta de NS224. A substituição da Mídia de inicialização pode ser feita sem interrupções, enquanto o compartimento de unidades está ligado e e e/S está em andamento. 
---
= Substitua a mídia de inicialização do NS224
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
