...
summary: 'Os gabinetes de discos com módulos IOM12, IOM12B ou IOM12C podem ser cabeados em configurações de par de HA e de controlador único (para plataformas compatíveis).' 
---
= Regras e conceitos de cabeamento SAS para os shelves DS212C, DS224C ou DS460C
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Os shelves DS212C, DS224C e DS460C com módulos IOM12, IOM12B ou IOM12C podem ser cabeados em configurações de par de HA e de controlador único (para plataformas suportadas) aplicando as regras de cabeamento SAS: regras de configuração, regras de numeração de slots do controlador, regras de conexão entre shelves, regras de conexão entre controlador e stack e, se aplicável, regras de cabo óptico mini-SAS HD SAS.


NOTE: As regras de cabeamento SAS referentes à numeração dos slots do controlador, às regras de conexão entre gabinetes e às regras de conexão do controlador ao stack, descritas neste guia, são as mesmas regras que se aplicam a todos os gabinetes de discos SAS, independentemente de possuírem módulos IOM12, IOM12B ou IOM12C. No entanto, as informações neste guia são específicas para as características únicas dos gabinetes de discos com módulos IOM12, IOM12B ou IOM12C e seu uso em configurações suportadas.

As regras de cabeamento SAS referentes às regras de configuração e às regras de cabos ópticos mini-SAS HD SAS descritas neste guia são específicas para gabinetes de discos com módulos IOM12, IOM12B ou IOM12C.
...
Os gabinetes de discos com módulos IOM12, IOM12B ou IOM12C são suportados em tipos específicos de par de HA e configurações de controladores únicos.
