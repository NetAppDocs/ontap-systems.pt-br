...
summary: Você pode adicionar dinamicamente uma ou mais gavetas de discos a uma pilha existente de gavetas de discos ou adicionar dinamicamente uma pilha de uma ou mais gavetas de discos diretamente a um SAS HBA ou a uma porta SAS integrada no controlador. 
---
= Adicionar uma prateleira DS212C, DS224C ou DS460C com o equipamento em funcionamento
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Você pode adicionar dinamicamente uma ou mais gavetas de discos a uma pilha existente de gavetas de discos ou adicionar dinamicamente uma pilha de uma ou mais gavetas de discos diretamente a um SAS HBA ou a uma porta SAS integrada no controlador.
...
* Certifique-se de que seu sistema atenda a determinados requisitos antes de adicionar gavetas de disco com módulos IOM12, IOM12B ou IOM12C com o sistema ligado:
+
...
+
** Se você estiver adicionando dinamicamente um gabinete de discos com módulos IOM12, IOM12B ou IOM12C a uma pilha existente (de gabinetes de discos com módulos IOM12, IOM12B ou IOM12C), você pode adicionar dinamicamente o gabinete de discos a qualquer uma das extremidades, o primeiro ou o último gabinete de discos lógico, da pilha.
+
...
** Os compartimentos de discos com módulos IOM12, IOM12B ou IOM12C devem estar em sua própria pilha exclusiva.
...
CAUTION: *Possível perda de acesso aos dados:* Se, no futuro, você mover o shelf para uma parte diferente do data center ou transportá-lo para um local diferente, deverá remover os discos das gavetas de disco para evitar possíveis danos às gavetas de disco e aos discos.
...
+
Você desativa a atribuição automática de propriedade do disco antes de conectar os gabinetes de discos adicionados a quente e, em seguida, a reativa posteriormente neste procedimento, após a conexão dos gabinetes de discos adicionados a quente.
...
. Se você estiver adicionando dinamicamente uma pilha de gavetas de disco diretamente a um controlador, conclua as seguintes subetapas; caso contrário, vá para a próxima etapa.
+
...
Se no futuro você mover os shelves DS460C para outra parte do data center ou transportar os shelves para um local diferente, será necessário remover os discos das gavetas de disco para evitar possíveis danos às gavetas de disco e aos discos.
