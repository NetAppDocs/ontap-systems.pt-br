...
summary: Altere o ID de um shelf em um sistema quando o ONTAP ainda não está em execução, ao adicionar um shelf a quente antes de ele ser conectado ao sistema, ou quando o ONTAP está em funcionamento e todas as unidades no shelf não têm proprietário, são sobressalentes ou fazem parte de agregados off-line. 
---
= Altere o ID da shelf DS212C, DS224C ou DS460C
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Você pode alterar o ID de um shelf em um DS212C, DS224C ou DS460C quando ONTAP ainda não estiver em execução ou ao adicionar um shelf a quente antes de ele ser conectado ao sistema. Você também pode alterar o ID de um shelf quando ONTAP estiver em funcionamento (módulos controladores disponíveis para fornecer dados) e todas as unidades no shelf estiverem sem proprietário, forem reservas ou fizerem parte de agregados offline.
...
. Repita os passos anteriores para cada compartimento de disco adicional.
