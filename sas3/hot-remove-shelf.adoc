...
summary: Você pode remover a quente um compartimento de discos quando ele precisar ser removido ou substituído de um sistema que está ligado e com I/O em andamento. 
---
= Remova com o equipamento ligado uma shelf DS212C, DS224C ou DS460C
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Você pode remover um shelf de discos a quente quando ele precisar ser removido ou substituído em um sistema que esteja ligado e com operações de E/S em andamento. Você pode remover um ou mais shelves de discos a quente de qualquer ponto dentro de uma pilha de shelves de discos ou remover uma pilha de shelves de discos.
...
.. Se necessário, avance para a seção sobre como reativar a atribuição automática (vários passos abaixo).
...
+
** Os módulos IOM12, IOM12B ou IOM12C são dispostos lado a lado, como em um shelf de discos DS224C ou DS212C; se você tiver um DS460C, os módulos IOM12, IOM12B ou IOM12C são dispostos um acima do outro.
...
Se você mover um gabinete DS460C para outra parte do data center ou transportá-lo para um local diferente, deverá remover os discos das gavetas de disco para evitar possíveis danos às gavetas de disco e aos discos.
