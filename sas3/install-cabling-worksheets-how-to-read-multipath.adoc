...
summary: Use este exemplo para orientar você na leitura e aplicação de uma planilha preenchida às conexões de controlador para stack de prateleiras de discos para conectividade multipath. 
---
= Consulte uma folha de instruções de cabeamento para conexões multipath de controlador para pilha DS212C, DS224C ou DS460C
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Você pode usar este exemplo como guia para ler e aplicar uma planilha preenchida às conexões do controlador ao stack de gabinetes de discos com módulos IOM12, IOM12B ou IOM12C para conectividade multipath.
...
+
A configuração usada neste exemplo é uma configuração multipath de par de HA com dois HBAs SAS de quatro portas (oito portas SAS) em cada controlador e duas pilhas de gabinetes de discos com módulos IOM12, IOM12B ou IOM12C. Os pares de portas são conectados pulando cada outro par de portas na planilha.
