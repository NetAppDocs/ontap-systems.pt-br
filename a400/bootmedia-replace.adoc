...
summary: Você deve localizar a mídia de inicialização no módulo controlador e, em seguida, seguir as instruções para substituí-la. 
---
= Substitua a mídia de inicialização no seu sistema AFF A400 usando o processo manual de recuperação de inicialização
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
