...
summary: Após concluir a substituição da FRU em uma configuração MetroCluster de dois nós, execute a operação de reversão MetroCluster para retornar a configuração ao seu estado operacional normal, com as máquinas virtuais de armazenamento de origem de sincronização no site anteriormente afetado agora ativas e fornecendo dados dos pools de disco locais. 
---
= Reative os agregados em uma configuração de dois nós MetroCluster no seu sistema AFF A400
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
