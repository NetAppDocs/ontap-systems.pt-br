...
summary: Substitua um DIMM no seu módulo de controlador AFF A400 quando o sistema registrar um número crescente de códigos de correção de erro corrigíveis ou erros ECC incorrigíveis. A não substituição de um DIMM defeituoso pode causar um pânico no sistema. 
---
= Substitua um módulo DIMM no seu sistema AFF A400 para corrigir erros de memória
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Substitua um módulo DIMM no seu sistema AFF A400 quando o sistema de storage encontrar erros de memória corrigíveis (CECC) ou incorrigíveis em excesso. Esses erros podem impedir que o sistema de storage inicialize o ONTAP.
