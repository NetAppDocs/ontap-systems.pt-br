...
---
= Recable o sistema e reatribuir discos - FAS8200
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
... Aguarde até que `savecore` o comando seja concluído antes de emitir o giveback.
+
Pode introduzir o seguinte comando para monitorizar o progresso do `savecore` comando: `system node run -node _local-node-name_ partner savecore -s`
