...
summary: Substitua a mídia de inicialização com falha em um módulo controlador FAS8200 . O sistema FAS8200 suporta apenas procedimentos manuais de recuperação de mídia de inicialização. A recuperação automática de mídia de inicialização não é suportada. 
---
= Substitua o suporte de arranque - FAS8200
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
NOTE: O arquivo tar.gz deve ser colocado em uma partição formatada FAT32 que seja no mínimo 4GB. Embora as partições FAT32 possam ser tão grandes quanto 2TB, as ferramentas internas do Windows (por exemplo, diskpart) não podem formatar partições FAT32 maiores que 32GB.
