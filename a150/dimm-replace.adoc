...
summary: Você deve substituir um DIMM no controlador quando seu sistema de storage encontrar erros como erros CECC excessivos com base em alertas do Health Monitor ou erros ECC incorrigíveis, normalmente causados por uma única falha de DIMM que impede o sistema de storage de inicializar o ONTAP. 
---
= Substitua um DIMM no seu AFF A150 para corrigir erros de memória
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Substitua um DIMM no seu sistema AFF A150 quando o sistema de storage encontrar erros de memória corrigíveis (CECC) ou incorrigíveis em excesso. Esses erros podem impedir que o sistema de storage inicialize o ONTAP.
