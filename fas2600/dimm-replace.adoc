...
summary: 'Você deve substituir um DIMM no controlador quando seu sistema de storage encontrar erros como erros excessivos de Códigos de Correção de Erros Corrigíveis (ECC) com base em alertas do Health Monitor ou erros ECC incorrigíveis, normalmente causados por uma única falha de DIMM que impede o sistema de storage de inicializar o ONTAP.' 
---
