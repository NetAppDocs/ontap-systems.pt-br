...
summary: Restaure a criptografia na mídia de inicialização de substituição. Conclua as etapas específicas para sistemas que têm o Onboard Key Manager, NetApp Storage Encryption ou NetApp Volume Encryption ativados. 
---
= Restaure a criptografia na mídia de inicialização do seu sistema AFF A800 após o processo de recuperação manual de inicialização
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Restaure a criptografia na mídia de inicialização de substituição do seu sistema AFF A800 para garantir a proteção contínua dos dados. O processo de substituição envolve a verificação da disponibilidade da chave, a reaplicação das configurações de criptografia e a confirmação do acesso seguro aos seus dados.
