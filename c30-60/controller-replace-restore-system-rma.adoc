...
summary: Para concluir a substituição do controlador do seu sistema de armazenamento AFF C30 ou AFF C60, primeiro restaure a configuração do NetApp Storage Encryption, se necessário, e instale as licenças necessárias no novo controlador.  Em seguida, confirme se as interfaces lógicas LIFs estão reportando às suas portas iniciais e execute uma verificação de integridade do cluster.  Por fim, registre o número de série do novo controlador e devolva a peça com falha à NetApp 
---
= Conclua a substituição do controlador do seu sistema de storage AFF C30 ou AFF C60
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
