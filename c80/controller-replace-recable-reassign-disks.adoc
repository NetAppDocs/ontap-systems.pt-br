...
---
= Recable e devolver o controlador - AFF C80
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Retorne o controle dos recursos de armazenamento ao controlador de substituição para que seu sistema AFF C80 possa retomar a operação normal. O procedimento de devolução varia de acordo com o tipo de encriptação utilizado pelo seu sistema: sem encriptação, encriptação OKM (Onboard Key Manager) ou encriptação EKM (External Key Manager).
