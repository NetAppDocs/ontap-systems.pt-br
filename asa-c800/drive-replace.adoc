...
summary: Substitua uma unidade no seu ASA C800 quando a plataforma registrar uma mensagem de aviso no console do sistema indicando que uma unidade com falha foi detectada. Além disso, tanto o LED de falha no painel de exibição do operador quanto o LED de falha na unidade com falha estarão acesos. 
---
= Substitua uma unidade de disco no seu sistema ASA C800
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Substitua uma unidade no seu sistema ASA C800 quando uma unidade falhar ou precisar de uma atualização. O processo de substituição envolve identificar a unidade defeituosa, removê-la com segurança e instalar uma nova unidade para garantir o acesso contínuo aos dados e o desempenho do sistema.
