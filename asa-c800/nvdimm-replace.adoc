...
summary: Você deve substituir o NVDIMM no módulo controlador quando o sistema registrar que a vida útil da memória flash está quase no fim ou que o NVDIMM identificado não está saudável. A falha em fazer isso causa um pânico do sistema. 
---
= Substitua um NVDIMM no seu sistema ASA C800
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Substitua o NVDIMM no seu sistema ASA C800 quando o sistema detectar que a vida útil da memória flash está quase no fim ou que o NVDIMM identificado não está em boas condições; caso contrário, o sistema entrará em pânico.

.Antes de começar
* Certifique-se de que o módulo NVDIMM de substituição que você recebeu da NetApp é compatível com o seu sistema ASA C800 .
