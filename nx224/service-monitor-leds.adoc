...
---
= Prateleira de acionamento do monitor LEDs - prateleiras NX224
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Você pode monitorar a integridade da sua prateleira entendendo a localização e as condições de status dos LEDs nos componentes da prateleira da unidade.

* Os LEDs de localização (azuis), no painel de exibição do operador (ODP) de uma prateleira e em ambos os módulos de prateleira NVMe (NSMs), podem ser ativados para auxiliar na localização física da prateleira que precisa de manutenção: `storage shelf location-led modify -shelf-name _shelf_name_ -led-status on`
+
...
image::../media/drw_tp_nsmb_leds_ieops-2206.svg[Módulo LEDs NX224 NSM140]
...
Os LEDs em uma fonte de alimentação CA (PSU) indicam se a PSU está funcionando normalmente ou se há problemas de hardware.

A ilustração e as tabelas a seguir descrevem o LED em uma PSU.

image::../media/drw_generic_psu_ac_terra_cotta_ieops-2370.svg[LED de atividade de alimentação da fonte de alimentação]
...
Ligado
 ...
 a| 
Alimentação CA ligada, mas a PSU não está no gabinete
 a| 
Pisca
 a| 
Desligado
