...
summary: 'Você pode alterar uma ID de prateleira NX224 em um sistema quando o ONTAP ainda não estiver em execução ou ao adicionar uma prateleira antes de ela ser cabeada ao sistema.  Você também pode alterar uma ID de prateleira quando o ONTAP estiver ativo e em execução (os módulos do controlador estão disponíveis para fornecer dados) e todas as unidades na prateleira não forem proprietárias ou forem sobressalentes.' 
---
