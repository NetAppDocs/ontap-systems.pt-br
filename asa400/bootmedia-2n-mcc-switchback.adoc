...
summary: 'Após concluir a substituição da FRU em uma configuração de dois nós MetroCluster, você pode executar a operação de reversão MetroCluster. Isso retorna a configuração ao seu estado operacional normal, com as máquinas virtuais de storage de origem de sincronização (SVMs) no site anteriormente afetado agora ativas e fornecendo dados dos pools de discos locais.' 
---
= Reative os agregados em uma configuração de dois nós MetroCluster no seu sistema ASA A400
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
