...
summary: Comece a substituir a mídia de inicialização do seu sistema de storage AFF C250 revisando os requisitos de substituição, verificando o status da criptografia, desligando o controlador, substituindo a mídia de inicialização, inicializando a imagem de recuperação, restaurando a criptografia e verificando a funcionalidade do sistema. 
---
= Fluxo de trabalho de substituição da mídia de inicialização do AFF C250 usando o processo de recuperação manual de inicialização
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
