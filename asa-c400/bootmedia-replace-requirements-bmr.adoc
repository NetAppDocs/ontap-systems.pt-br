...
summary: Antes de substituir a mídia de inicialização do seu ASA C400, certifique-se de atender aos requisitos necessários para uma substituição bem-sucedida. Isso inclui verificar se você possui a mídia de inicialização de substituição correta, confirmar se a porta de chave inglesa e0S ou e0M no controlador com defeito não está com problemas e determinar se Onboard Key Manager ou External Key Manager está habilitado. 
---
= Analise os requisitos para recuperação automática da mídia de inicialização no seu sistema ASA C400
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
