...
summary: Você deve substituir o NVDIMM no módulo controlador quando o sistema registrar que a vida útil da memória flash está quase no fim ou que o NVDIMM identificado não está saudável em geral. A falha em fazê-lo causa um pânico do sistema. 
---
= Substitua um NVDIMM no seu sistema ASA C400
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Substitua um NVDIMM no seu sistema ASA C400 quando o sistema de storage apresentar um número excessivo de erros ECC corrigíveis ou erros ECC não corrigíveis, que normalmente são causados por uma única falha de NVDIMM.
