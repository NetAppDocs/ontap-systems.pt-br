...
summary: Substitua um DIMM no módulo do controlador ASA C400 quando o sistema registrar um número crescente de códigos de correção de erros corrigíveis ou erros ECC incorrigíveis. A não substituição de um DIMM defeituoso pode causar um pânico no sistema. 
---
= Substitua um DIMM no seu ASA C400 para corrigir erros de memória
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Substitua um DIMM no seu ASA C400 quando o sistema de storage apresentar um número excessivo de erros de código de correção de erros (ECC) corrigíveis ou um erro de ECC não corrigível, o que normalmente é causado por uma falha em um único DIMM.
