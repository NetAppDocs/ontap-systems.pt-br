...
summary: Você deve substituir o NVDIMM no módulo controlador quando o sistema registrar que a vida útil da memória flash está quase no fim ou que o NVDIMM identificado não está em boas condições, ou a falha em fazê-lo causará um pânico no sistema. 
---
= Substitua um módulo NVDIMM no seu sistema de storage AFF C400
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Substitua um NVDIMM no seu sistema de storage AFF C400 quando o sistema registrar um número excessivo de erros ECC corrigíveis ou um erro ECC não corrigível, o que normalmente é causado por uma falha em um único NVDIMM.
