...
summary: 'Você deve substituir um módulo DIMM no controlador quando o sistema registrar um número crescente de códigos de correção de erros (ECC) corrigíveis ou, caso contrário, isso causará uma pane no sistema.' 
---
= Substitua um módulo DIMM no seu sistema de storage AFF C400 para corrigir erros de memória
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Substitua um DIMM no seu sistema de storage AFF C400 quando o sistema apresentar um número excessivo de erros ECC corrigíveis ou um erro ECC não corrigível, o que normalmente é causado pela falha de um único DIMM.
