...
summary: Substitua um DIMM no seu AFF A220 controller module quando o sistema registrar um número crescente de códigos de correção de erro corrigíveis ou erros ECC incorrigíveis. A não substituição de um DIMM defeituoso pode causar um pânico no sistema. 
---
= Substitua um módulo DIMM no seu sistema AFF A220
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Substitua um DIMM no seu sistema AFF A220 quando o sistema de storage apresentar um número excessivo de erros ECC corrigíveis ou um erro ECC incorrigível, o que normalmente é causado pela falha de um único DIMM.
