...
summary: 'Após verificar as variáveis ​​de ambiente, você deve concluir as etapas específicas para sistemas que tenham o Onboard Key Manager (OKM), o NetApp Storage Encryption (NSE) ou o NetApp Volume Encryption (NVE) ativados. O sistema AFF A220 suporta apenas procedimentos manuais de recuperação de mídia de inicialização.' 
---
= Restaurar encriptação - AFF A220
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Restaure a criptografia na mídia de inicialização de substituição. O sistema AFF A220 suporta apenas procedimentos manuais de recuperação de mídia de inicialização. A recuperação automática de mídia de inicialização não é suportada.
