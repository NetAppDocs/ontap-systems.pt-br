...
summary: Você deve localizar a mídia de inicialização no controlador e seguir as instruções para substituí-la. O sistema AFF A220 suporta apenas procedimentos manuais de recuperação de mídia de inicialização. 
---
= Substitua a mídia de inicialização no seu sistema AFF A220
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
