...
summary: Para restaurar a mídia de inicialização, inicie o sistema AFF C190 a partir da imagem de recuperação do ONTAP na unidade USB. O sistema AFF C190 suporta apenas procedimentos manuais de recuperação de mídia de inicialização. A recuperação automática de mídia de inicialização não é suportada. 
---
= Inicialize a imagem de recuperação no seu sistema AFF C190
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Para restaurar a mídia de inicialização, inicie o sistema AFF C190 a partir da imagem de recuperação do ONTAP na unidade USB. Este procedimento inclui inicializar a partir da unidade flash USB, restaurar o sistema de arquivos, verificar as variáveis ​​de ambiente e retornar o controlador à operação normal após a substituição da mídia de inicialização.
...
.Passos
. A partir do prompt Loader, inicialize a imagem de recuperação da unidade flash USB:
+
`*boot_recovery*`
