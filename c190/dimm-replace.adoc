...
summary: 'Você deve substituir um módulo DIMM no controlador quando seu sistema de storage encontrar erros como erros excessivos de código de correção corrigíveis (ECC) com base em alertas do Health Monitor ou erros ECC não corrigíveis, normalmente causados por uma única falha de DIMM que impede o sistema de storage de inicializar o ONTAP.' 
---
= Substitua um DIMM no seu AFF C190 para corrigir erros de memória
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Para substituir os DIMMs, você precisa localizá-los dentro do módulo do controlador e, em seguida, seguir a sequência específica de etapas.
