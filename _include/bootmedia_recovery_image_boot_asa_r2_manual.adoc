...
--
.. No prompt DO Loader no controlador prejudicado, configure o endereço IP do módulo do controlador prejudicado:
+
`ifconfig e0M - auto`

.. No prompt Loader, netboot o nó prejudicado:
+
`netboot \http://_web_server_ip/path_to_web-accessible_directory_/<ontap_version>_image.tgz`

.. No menu Boot (Inicialização), selecione Option(opção `7`).
.. Digite `n` quando solicitado a restaurar a configuração de backup.
.. Digite `y` quando solicitado a reinicializar o nó.
.. Prima `ctrl-c` para regressar ao Menu de arranque.
+
Se a encriptação estiver ativada, tem de a restaurar antes de continuar a restaurar a imagem de arranque. Vá para link:bootmedia-encryption-restore.html["Restaure a criptografia"].
