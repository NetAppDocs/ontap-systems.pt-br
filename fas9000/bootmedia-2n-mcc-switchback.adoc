...
summary: 'Após concluir a substituição da FRU em uma configuração MetroCluster de dois nós, você pode executar a operação de reversão MetroCluster. Isso retorna a configuração ao seu estado operacional normal, com as máquinas virtuais de storage de origem de sincronização (SVMs) no site anteriormente afetado agora ativas e fornecendo dados dos pools de discos locais. O sistema FAS9000 suporta apenas procedimentos manuais de recuperação da mídia de inicialização.' 
---
