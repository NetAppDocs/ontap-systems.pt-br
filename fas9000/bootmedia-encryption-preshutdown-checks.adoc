...
---
= Verifique as chaves de criptografia integradas
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
. Se o nó prejudicado fizer parte de uma configuração de HA, desative a giveback automática do nó íntegro: `storage failover modify -node local -auto-giveback false` Ou `storage failover modify -node local -auto-giveback-after-panic false`
