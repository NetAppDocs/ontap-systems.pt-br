...
summary: Para acessar os componentes internos do controlador, primeiro você deve remover o módulo do controlador do sistema e, em seguida, remover a tampa do módulo do controlador. O sistema FAS9000 suporta apenas procedimentos manuais de recuperação de mídia de inicialização. 
---
= Substitua o suporte de arranque - FAS9000
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Para substituir a mídia de inicialização, você deve remover o módulo controlador defeituoso, instalar a mídia de inicialização de substituição e transferir a imagem de inicialização para uma unidade flash USB. O sistema FAS9000 suporta apenas procedimentos manuais de recuperação de mídia de inicialização. A recuperação automática de mídia de inicialização não é suportada.
