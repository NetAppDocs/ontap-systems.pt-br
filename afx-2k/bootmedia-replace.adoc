...
summary: A mídia de inicialização do seu sistema de storage AFX 2K armazena firmware essencial e dados de configuração. O processo de substituição envolve remover o módulo de gerenciamento do sistema, remover a mídia de inicialização com problema, instalar a mídia de inicialização de substituição no módulo de gerenciamento do sistema e, em seguida, reinstalar o módulo de gerenciamento do sistema. 
---
= Substitua a mídia de inicialização no seu sistema de storage AFX 2K usando o processo automatizado de recuperação de inicialização.
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
.. Remova os cabos do módulo de gerenciamento do sistema e etiquete-os para garantir a reconexão correta durante a reinstalação.
...
.O que se segue?
Após substituir a mídia de inicialização,link:bootmedia-recovery-image-boot.html["Restaure a imagem do ONTAP a partir do nó do parceiro"] .
