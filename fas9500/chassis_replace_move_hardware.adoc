...
Para remover os módulos de e/S do chassis afetado, incluindo os módulos NVRAM, siga a sequência específica dos passos. Você não precisa remover o módulo FlashCache, se presente, do módulo NVRAM ao movê-lo para um chassi de substituição.
...
Para instalar módulos de e/S, incluindo os módulos NVRAM/FlashCache a partir do chassi com deficiência, siga a sequência específica de etapas.
