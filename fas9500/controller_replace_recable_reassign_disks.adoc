...
summary: Recabeie o sistema e reatribua os discos. 
---
= Recable o sistema - FAS9500
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
. Aguarde até que a `Waiting for giveback...` mensagem seja exibida no console do nó _replacement_ e, em seguida, a partir do nó de integridade, verifique se a nova ID do sistema do parceiro foi atribuída automaticamente: Exibição de failover de armazenamento
+
...
.. Após a conclusão do giveback, confirme se o par de HA está íntegro e se o takeover é possível: Mostra failover de storage
+
