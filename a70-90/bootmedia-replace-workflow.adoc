...
---
= Fluxo de trabalho de substituição de Mídia de inicialização - AFF A70 e AFF A90
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Comece a substituir a Mídia de inicialização em seu sistema de armazenamento AFF A70 ou AFF A90, revisando os requisitos de substituição, verificando o status da criptografia, desligando o controlador, substituindo a Mídia de inicialização, inicializando a imagem de recuperação, restaurando a criptografia e verificando a funcionalidade do sistema.
