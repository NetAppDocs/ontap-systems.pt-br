...
---
= Fluxo de trabalho de substituição da mídia de inicialização do AFF A70 e AFF A90 usando o processo de recuperação de inicialização manual
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Se o seu sistema de armazenamento estiver executando o ONTAP 9.17.1 ou posterior, use olink:bootmedia-replace-workflow-bmr.html["procedimento automatizado de recuperação de inicialização"] .  Se o seu sistema estiver executando uma versão anterior do ONTAP, use o procedimento de recuperação de inicialização manual.
