...
[role="lead"]
Substitua o chassi do sistema AFF A70 ou AFF A90 quando uma falha ou atualização de hardware exigir isso. O processo de substituição envolve a remoção das unidades de fonte de alimentação (PSUs), a remoção do chassi existente, a instalação do novo e a reinstalação dos componentes do chassi.
