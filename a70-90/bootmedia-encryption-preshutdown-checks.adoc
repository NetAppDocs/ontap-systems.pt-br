...
summary: Para garantir a segurança dos dados no seu AFF A70 ou AFF A90 sistema de storage, você precisa verificar o suporte e o status da chave de criptografia na sua mídia de inicialização. Verifique se a sua versão do ONTAP suporta NetApp Volume Encryption e, antes de desligar o controlador, verifique se o gerenciador de chaves está ativo. 
---
= Verifique o suporte de criptografia para recuperação manual de mídia de inicialização - AFF A70 e AFF A90
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
