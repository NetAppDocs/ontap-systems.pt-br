...
summary: Comece a substituir a mídia de inicialização no seu sistema de armazenamento FAS2800 revisando os requisitos de substituição, verificando o status da criptografia, desligando o controlador, substituindo a mídia de inicialização, inicializando a imagem de recuperação, restaurando a criptografia e verificando a funcionalidade do sistema. 
---
= Fluxo de trabalho de recuperação manual de mídia de inicialização - FAS2800
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
