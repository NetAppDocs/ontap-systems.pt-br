...
summary: 'Retorne o controle dos recursos de armazenamento ao controlador de substituição para que seu sistema de armazenamento ASA A20, ASA A30 ou ASA A50 possa retomar a operação normal.  O procedimento de devolução varia de acordo com o tipo de criptografia usado pelo seu sistema: sem criptografia, criptografia do Onboard Key Manager (OKM) ou criptografia do External Key Manager (EKM).' 
---
