...
summary: A mídia de inicialização do seu sistema de armazenamento ASA A20, ASA A30 ou ASA A50 armazena dados essenciais de firmware e configuração. O processo de substituição envolve a remoção do módulo controlador, a remoção da mídia de inicialização danificada, a instalação da mídia de inicialização de substituição e a reinstalação do módulo controlador. 
---
= Substitua a mídia de inicialização no seu sistema ASA A20, ASA A30 ou ASA A50 usando o processo automatizado de recuperação de inicialização
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
