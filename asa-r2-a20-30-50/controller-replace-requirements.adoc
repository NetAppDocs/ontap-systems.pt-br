...
[role="lead"]
Antes de substituir o controlador no sistema ASA A20, ASA A30 ou ASA A50, certifique-se de que cumpre os requisitos necessários para uma substituição bem-sucedida. Isso inclui verificar se todos os outros componentes do sistema estão funcionando corretamente, verificar se você tem o controlador de substituição correto e salvar a saída do console do controlador em um arquivo de log de texto.
...
.O que se segue?
Depois de analisar os requisitos para substituir a controladora ASA A20, ASA A30 ou ASA A50, é necessário link:controller-replace-shutdown.html["desligue o controlador desativado"].
