...
summary: Retorne o controle dos recursos de storage ao controlador de substituição para que seu sistema ASA A70 ou ASA A90 possa retomar a operação normal. 
---
= Devolva o controlador no seu sistema ASA A70 ou ASA A90 para retomar as operações normais
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Retorne o controle dos recursos de storage ao controlador de substituição para que seu sistema ASA A70 ou ASA A90 possa retomar a operação normal. O procedimento de devolução varia de acordo com o tipo de criptografia usado pelo seu sistema: sem criptografia ou criptografia Onboard Key Manager (OKM).

[role="tabbed-block"]
====
.Sem criptografia ou External Key Manager (EKM)
--
...
--
Restaure e vede novamente as chaves de criptografia integradas do OKM e retorne o controlador à operação normal.

.Antes de começar
Antes de começar, revise  https://docs.netapp.com/us-en/ontap-cli/security-key-manager-onboard-show-backup.html["security key-manager onboard show-backup"] ou entre em contato com o suporte da NetApp.

.Passos
. Inicie no menu ONTAP a partir do prompt Loader `boot_ontap menu` e selecione a opção 10.
. Exibir as informações de backup de gerenciamento de chaves para o cluster.
+
.. Execute o seguinte comando para exibir as informações de backup do gerenciamento de chaves para o cluster:
+
`security key-manager onboard show-backup`

.. Execute o seguinte comando para verificar o backup do onboard key manager:
+
`security key-manager onboard verify-backup`



. Digite a senha do OKM e confirme a verificação bem-sucedida.
+
...
.. Verifique o status das chaves: `security key-manager key query -restored false`
+
Quando a sincronização estiver correta, a saída não deverá apresentar resultados.
...
NOTE: Não ignore o veto no giveback para chaves ausentes. Entre em contato com o suporte da NetApp.
