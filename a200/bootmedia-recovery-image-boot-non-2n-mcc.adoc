...
---
= Inicialize a imagem de recuperação no seu sistema AFF A200
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Você deve inicializar a imagem do ONTAP a partir da unidade USB, restaurar o sistema de arquivos e verificar as variáveis ​​de ambiente. O sistema AFF A200 suporta apenas procedimentos manuais de recuperação de mídia de inicialização. A recuperação automática de mídia de inicialização não é suportada.
