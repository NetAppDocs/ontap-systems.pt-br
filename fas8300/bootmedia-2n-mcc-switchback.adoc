...
summary: Após concluir a substituição da FRU em uma configuração MetroCluster de dois nós, você pode executar a operação de reversão MetroCluster. Isso restaura a configuração ao seu estado operacional normal, com as máquinas virtuais de armazenamento de origem de sincronização no site anteriormente afetado agora ativas e fornecendo dados dos pools de discos locais. 
---
= Agregados de comutação reversa em uma configuração MetroCluster de dois nós - FAS8300 e FAS8700
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
