...
[role="lead"]
Para concluir a substituição do controlador para o seu sistema ASA A1K, primeiro restaure a configuração de encriptação de armazenamento NetApp (se necessário) e instale as licenças necessárias no novo controlador. Em seguida, confirme se as interfaces lógicas (LIFs) estão relatando para suas portas domésticas e execute uma verificação de integridade do cluster. Finalmente, Registre o número de série do novo controlador e, em seguida, devolva a peça com falha ao NetApp.
