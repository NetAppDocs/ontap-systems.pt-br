...
[role="lead"]
Retorne o controle dos recursos de armazenamento ao controlador de substituição para que seu sistema ASA A1K possa retomar a operação normal. O procedimento de devolução varia de acordo com o tipo de encriptação utilizado pelo seu sistema: sem encriptação, encriptação OKM (Onboard Key Manager) ou encriptação EKM (External Key Manager).
