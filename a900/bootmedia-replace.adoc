...
---
= Substitua a mídia de inicialização no seu sistema AFF A900 usando o processo manual de recuperação de inicialização
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
A mídia de inicialização do seu AFF A900 armazena dados essenciais de firmware e configuração. O processo de substituição envolve remover o módulo do controlador, remover a mídia de inicialização com problema, instalar a mídia de inicialização de substituição e, em seguida, transferir a imagem ONTAP para a mídia de inicialização de substituição usando um pen drive USB.
...
+
image::../media/drw_a900_remove_boot_dev.svg[Remova o suporte de arranque]
