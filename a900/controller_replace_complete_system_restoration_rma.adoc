...
summary: Para restaurar o funcionamento completo do seu sistema AFF A900, reconecte os cabos de armazenamento, restaure a configuração de criptografia de armazenamento NetApp, se necessário, e instale as licenças para o novo controlador. 
---
= Conclua a substituição do controlador do seu sistema AFF A900
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Para concluir a substituição do controlador do seu AFF A900, restaure a configuração de criptografia de storage da NetApp (se necessário), confirme se as interfaces lógicas (LIFs) estão reportando às suas portas de origem, execute uma verificação de integridade do cluster e devolva a peça com defeito para a NetApp.
