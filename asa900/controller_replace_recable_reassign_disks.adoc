...
summary: Recable as configurações de storage e rede, confirme a realocação de discos e devolva o controle dos recursos de storage ao controlador de substituição para que seu sistema ASA A900 possa retomar a operação normal. 
---
= Devolva o controlador no seu sistema ASA A900 para retomar as operações normais
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
