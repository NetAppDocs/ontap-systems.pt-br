...
summary: A mídia de inicialização do seu sistema ASA A900 armazena dados essenciais de firmware e configuração. O processo de substituição envolve a remoção do módulo do controlador, a remoção da mídia de inicialização danificada, a instalação da mídia de inicialização de substituição no módulo do controlador e, em seguida, a reinstalação do módulo do controlador. 
---
= Substitua a mídia de inicialização no seu sistema ASA A900 usando o processo automatizado de recuperação de inicialização
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
