...
summary: Você deve localizar a mídia de inicialização no módulo controlador e, em seguida, seguir as instruções para substituí-la. O sistema AFF A320 suporta apenas procedimentos manuais de recuperação de mídia de inicialização. A recuperação automática de mídia de inicialização não é suportada. 
---
= Saiba mais sobre a substituição da mídia de inicialização no seu sistema AFF A320
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
