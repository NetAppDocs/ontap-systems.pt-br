...
summary: Após verificar as variáveis ​​de ambiente, você deve concluir as etapas específicas para sistemas que tenham o Onboard Key Manager, o NetApp Storage Encryption ou o NetApp Volume Encryption ativados. O sistema AFF A320 suporta apenas procedimentos manuais de recuperação de mídia de inicialização. A recuperação automática de mídia de inicialização não é suportada. 
---
= Restaure a criptografia na mídia de inicialização do seu sistema AFF A320
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
