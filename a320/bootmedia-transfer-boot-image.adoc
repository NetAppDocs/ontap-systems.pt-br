...
summary: A mídia de inicialização de substituição que você instalou não possui uma imagem de inicialização, portanto, você precisa transferir uma imagem de inicialização usando um pen drive USB. O sistema AFF A320 suporta apenas procedimentos manuais de recuperação de mídia de inicialização. A recuperação automática de mídia de inicialização não é suportada. 
---
= Transfira a imagem de inicialização para a mídia de inicialização do seu sistema AFF A320 usando uma unidade flash USB
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
