...
summary: Para garantir a segurança dos dados no seu sistema de armazenamento FAS70 ou FAS90, você precisa verificar o suporte e o status da chave de criptografia na sua mídia de inicialização. Verifique se a sua versão do ONTAP é compatível com o NetApp Volume Encryption e, antes de desligar o controlador, verifique se o gerenciador de chaves está ativo. 
---
= Verifique o suporte de criptografia para recuperação manual de mídia de inicialização - FAS70 e FAS90
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
