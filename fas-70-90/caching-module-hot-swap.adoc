...
summary: 'O transportador do módulo NVMe SSD Flash Cache contém um ou dois módulos Flash Cache (módulos de cache) com uma única unidade de armazenamento em cache SSD integrada a cada módulo de armazenamento em cache. Os módulos de cache podem ser trocados a quente, mas o suporte do módulo Flash Cache não é trocado a quente.' 
---
...
[role="lead"]
O suporte do módulo NVMe SSD Flash Cache contém um ou dois módulos Flash Cache (módulos de cache) com uma única unidade SSD Flash Cache integrada a cada módulo de cache.
...
Você pode executar qualquer um dos seguintes procedimentos dependendo do que precisa ser substituído: Todo o suporte do módulo Flash Cache ou um módulo de cache.
