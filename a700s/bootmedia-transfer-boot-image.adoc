...
---
= Transfira a imagem de inicialização para a mídia de inicialização do seu sistema AFF A700s usando um pen drive USB
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Transfira a imagem de inicialização para a mídia de inicialização de substituição em um AFF A700s usando a mídia de inicialização secundária (método primário) ou uma unidade flash USB (caso a restauração da mídia de inicialização secundária falhe ou o  `image.tgz` arquivo esteja ausente).
...
== Opção 1: transferir a imagem do sistema usando recuperação de backup da segunda mídia de inicialização

Você pode instalar a imagem do sistema na mídia de inicialização de substituição usando a imagem presente na segunda mídia de inicialização do módulo controlador. Este é o método principal para sistemas com duas mídias de inicialização no módulo controlador.

A imagem na mídia de inicialização secundária deve conter um  `image.tgz` arquivo e não deve apresentar falhas. Se o  `image.tgz` arquivo estiver ausente ou a mídia de inicialização apresentar falhas, você não poderá usar este procedimento. Use <<Opção 2: Transfira a imagem de inicialização usando um pen drive USB.>> em vez disso.
...
Este procedimento só deve ser utilizado se a restauração da mídia de inicialização secundária falhar ou se o  `image.tgz` arquivo não for encontrado na mídia de inicialização secundária.

* Você deve ter uma unidade flash USB, formatada para FAT32, com pelo menos 4GBGB de capacidade.
* Uma cópia da mesma versão da imagem do ONTAP que estava sendo executada no controlador com problemas. Você pode baixar a imagem apropriada na seção de Downloads no site de suporte da NetApp.
+
...
.. Salve as variáveis de ambiente que você alterou com o comando  `savenv`.
...
. A partir do controlador parceiro no nível avançado de privilégio, inicie a sincronização de configuração utilizando o endereço IP registado na etapa anterior: `system node restore-backup -node local -target-address _impaired_node_IP_address_`
. Depois que a sincronização da configuração for concluída sem erros, pressione `y` quando solicitado para confirmar que o procedimento de backup foi bem-sucedido.
. Verifique se as variáveis de ambiente estão configuradas conforme o esperado.
+
