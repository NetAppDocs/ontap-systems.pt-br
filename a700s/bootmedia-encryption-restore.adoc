...
summary: Restaurar a configuração de criptografia na mídia de inicialização de substituição para um sistema AFF A700s . O sistema AFF A700s suporta apenas procedimentos manuais de recuperação de mídia de inicialização. A recuperação automática de mídia de inicialização não é suportada. 
---
= Restaure a criptografia na mídia de inicialização do seu sistema AFF A700s
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Restaurar a configuração de criptografia na mídia de inicialização de substituição para um sistema AFF A700s . Este procedimento inclui a conclusão das etapas pós-substituição para sistemas com Onboard Key Manager (OKM), NetApp Storage Encryption (NSE) ou NetApp Volume Encryption (NVE) ativados, para garantir o acesso seguro aos dados e o funcionamento adequado do sistema.
