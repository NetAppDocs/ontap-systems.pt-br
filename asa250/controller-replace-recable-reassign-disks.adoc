...
summary: Devolva o controle dos recursos de storage ao controlador de substituição para que seu sistema ASA A250 possa retomar a operação normal. 
---
= Devolva o controlador no seu sistema ASA A250 para retomar as operações normais
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Após concluir a restauração e verificar a configuração do sistema ASA A250, você precisa reconectar o sistema e reatribuir os discos.
...
.O que se segue?
Depois de reconectar o sistema e reatribuir os discos, você precisa link:controller-replace-restore-system-rma.html["complete a substituição do controlador"].
